...
export default function TransactionsPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchesSearch =
        !query ||
        [item.title, item.category, item.wallet, item.toWallet, item.note]
          .filter(Boolean)
          .some((value) => value.toLowerCase().includes(query));
      const matchesType = typeFilter === "All" || item.type === typeFilter;
      const matchesWallet =
        walletFilter === "All" || item.wallet === walletFilter || item.toWallet === walletFilter;
  ...
  const saveTransaction = () => {
    ...
    setItems((current) =>
      editingTransaction
        ? current.map((item) => (item.id === editingTransaction.id ? nextTransaction : item))
  ...
  const duplicateTransaction = (item) => {
    setMenuTransaction(null);
    setItems((current) => [
      { ...item, id: Date.now(), title: `${item.title} copy`, time: "Just now", dateLabel: "Today", date: "2026-09-08" },
      ...current,
    ]);
  };
    ...
    <>
      <section>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#9aa1ab]">Transaction history</p>
            <h2 className="mt-2 text-2xl font-bold tracking-[-0.04em] text-[#111827] sm:text-[30px]">Every movement, accounted for.</h2>
            <p className="mt-2 max-w-xl text-sm leading-6 text-[#7a828e]">Search, filter, and manage every income, expense, and transfer across your wallets.</p>
          </div>
          <Button
            variant="contained"
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[
            { label: "Income", value: summary.income, tone: "text-[#15803d]", icon: ArrowDownLeft },
            { label: "Expense", value: summary.expense, tone: "text-[#dc2626]", icon: ArrowUpRight },
            { label: "Net flow", value: summary.net, tone: summary.net >= 0 ? "text-[#15803d]" : "text-[#dc2626]", icon: ArrowDownUp },
            { label: "Transactions", value: summary.count, tone: "text-[#111827]", icon: Landmark, compact: true },
          ].map((stat) => {
            const Icon = stat.icon;
            return (
              <div key={stat.label} className="rounded-2xl border border-black/[0.06] bg-white p-4">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-[11px] font-medium text-[#8a919d]">{stat.label}</p>
                  <Icon size={15} className="text-[#9ca3af]" />
                </div>
                <p className={`mt-2 text-lg font-bold tracking-[-0.03em] ${stat.tone}`}>{stat.compact ? stat.value : money(stat.value)}</p>
              </div>
            );
          })}
        </div>
      ...
      <section className="mt-6 rounded-[20px] border border-black/[0.06] bg-white p-4 md:p-5">
        ...
        {(search || typeFilter !== "All" || walletFilter !== "All") && (
          <div className="mt-3 flex items-center justify-between gap-4 border-t border-black/[0.05] pt-3">
            <p className="text-[11px] text-[#9ca3af]">{filteredItems.length} matching transactions</p>
            <button type="button" onClick={() => { setSearch(""); setTypeFilter("All"); setWalletFilter("All"); }} className="cursor-pointer text-[11px] font-semibold text-[#6b7280] transition hover:text-[#111827]">Reset filters</button>
          </div>
        )}
      </section>

      <section className="mt-5 rounded-[20px] border border-black/[0.06] bg-white px-4 py-2 md:px-5">
        {filteredItems.length ? (
          Object.entries(groupedItems).map(([group, groupItems]) => (
            <div key={group} className="py-3">
              <p className="px-1 pb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#9ca3af]">{group}</p>
                ...
                {groupItems.map((item, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="group flex cursor-pointer items-center gap-3 py-4 sm:gap-4"
                    >
                      <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl" style={{ backgroundColor: meta.bg }}><Icon size={17} strokeWidth={1.8} /></div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex min-w-0 items-center gap-2">
                              <p className="truncate text-sm font-semibold text-[#111827]">{item.title}</p>
                              <span className={`hidden rounded-full px-2 py-0.5 text-[9px] font-semibold sm:inline ${typeTone[item.type]}`}>{item.type}</span>
                            </div>
                          ...
                          <div className="flex shrink-0 items-start gap-2">
                            <div className="text-right">
                              <p className={`text-sm font-bold ${amountTone(item.type)}`}>{item.type === "Income" ? "+" : item.type === "Expense" ? "−" : ""}{money(Math.abs(item.amount))}</p>
                              <p className="mt-1 text-[10px] text-[#a1a7b0]">{item.type}</p>
                            </div>
                            <div className="relative" onClick={(event) => event.stopPropagation()}>
                              <button type="button" onClick={() => setMenuTransaction(menuTransaction?.id === item.id ? null : item)} className="grid h-8 w-8 cursor-pointer place-items-center rounded-lg text-[#9ca3af] transition hover:bg-[#f5f6f7] hover:text-[#111827]" aria-label={`More options for ${item.title}`}><MoreHorizontal size={17} /></button>
                              <AnimatePresence>
          ...
          <div className="py-16 text-center">
            <div className="mx-auto grid h-11 w-11 place-items-center rounded-2xl bg-[#f3f4f6] text-[#7a828e]"><Search size={18} /></div>
            <p className="mt-4 text-sm font-semibold text-[#111827]">No transactions found</p>
            <p className="mt-1 text-xs text-[#9ca3af]">Try changing your search or filters.</p>
          </div>
        )}
      </section>
      ...
      <Dialog open={formOpen} onClose={() => setFormOpen(false)} fullWidth maxWidth="sm" PaperProps={{ sx: { borderRadius: "22px" } }}>
        <DialogTitle sx={{ px: 3, pt: 3, pb: 1.5 }}>
          <div className="flex items-start justify-between gap-4">
            <div><p className="text-lg font-extrabold tracking-[-0.03em]">{editingTransaction ? "Edit transaction" : "Add transaction"}</p><p className="mt-1 text-xs font-normal text-[#9ca3af]">{editingTransaction ? "Update this money movement." : "Record a new income, expense, or transfer."}</p></div>
            <button type="button" onClick={() => setFormOpen(false)} className="grid h-8 w-8 cursor-pointer place-items-center rounded-lg text-[#9ca3af] hover:bg-[#f5f6f7]"><X size={17} /></button>
          </div>
        </DialogTitle>
        <DialogContent sx={{ px: 3, pb: 3 }}>
          <div className="grid gap-4 pt-1">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <TextField fullWidth label="Title" value={formTitle} onChange={(event) => setFormTitle(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} sx={fieldSx} />
              <TextField fullWidth label="Amount" placeholder="Rp0" value={formAmount} onChange={(event) => setFormAmount(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} sx={fieldSx} />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {formType === "Transfer" ? (
                <TextField select fullWidth label="To wallet" value={formToWallet} onChange={(event) => setFormToWallet(event.target.value)} sx={fieldSx}>{walletNames.filter((wallet) => wallet !== formWallet).map((wallet) => <MenuItem key={wallet} value={wallet}>{wallet}</MenuItem>)}</TextField>
              ) : (
                <TextField select fullWidth label="Category" value={formCategory} onChange={(event) => setFormCategory(event.target.value)} sx={fieldSx}>{Object.keys(categoryMeta).filter((category) => category !== "Transfer").map((category) => <MenuItem key={category} value={category}>{category}</MenuItem>)}</TextField>
              )}
            </div>
          ...
          <>
            ...
            <DialogContent sx={{ px: 3, pb: 3 }}>
              ...
              <div className="mt-5 space-y-3 text-sm">
                {[['Category', detailTransaction.category], ['Wallet', detailTransaction.type === 'Transfer' ? `${detailTransaction.wallet} → ${detailTransaction.toWallet}` : detailTransaction.wallet], ['Date', detailTransaction.date], ['Note', detailTransaction.note || 'No note']].map(([label, value]) => <div key={label} className="flex items-start justify-between gap-5 border-b border-black/[0.055] pb-3"><span className="text-[#9ca3af]">{label}</span><span className="text-right font-semibold text-[#111827]">{value}</span></div>)}
              </div>
